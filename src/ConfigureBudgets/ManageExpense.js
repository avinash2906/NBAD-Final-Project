 ...
 // const { isLoggedIn } = useAuth();
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const categoriesResponse = await axios.get(`https://nbad-pb-backend.onrender.com/get-categories/${userData}?month=${selectedMonth}`, {
          headers: {
  ...
  const handleAddExpense = async () => {
    try {
      ...
      await axios.post('https://nbad-pb-backend.onrender.com/add-expense', {
        userData: userData,
        month: selectedMonth,
  ...
  return (
    <main className="center" id="main" aria-label="main">
      <div style={{ width: '300px', margin: 'auto' }}>
        ...
        <button onClick={handleAddExpense} style={{ backgroundColor: 'blue', color: 'white', padding: '10px 15px', border: 'none', borderRadius: '5px', cursor: 'pointer', width: '100%', marginBottom: '15px' }}>Add Expense</button>
      </div>
    </main>
  );
